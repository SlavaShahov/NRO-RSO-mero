...
func TestDeadline_Sunday(t *testing.T) {
	// Вс 19 апр 2026:
	// subtractWorkdays(19апр, 3):
	//   18(Сб) — выходной, пропуск
	//   17(Пт) — рабочий [1]
	//   16(Чт) — рабочий [2]
	//   15(Ср) — рабочий [3]
	// дедлайн = 15 апр 23:59:59
...
func TestDeadline_DeadlineNeverOnWeekend(t *testing.T) {
	...
	for i := 0; i < 30; i++ {
		...
		if wd == time.Saturday || wd == time.Sunday {
			t.Errorf("дедлайн для %v = %v (%s) — выходной!",
				event.Format("02.01"), dl.Format("02.01"), wd)
		}
	}
}
...
func TestSubtract_SkipsFullWeekend(t *testing.T) {
	// Пн 20 апр - 5 рабочих дней:
	// 20 -> 17(Пт)[1] -> 16(Чт)[2] -> 15(Ср)[3] -> 14(Вт)[4] -> 13(Пн)[5]
	mon  := date(2026, 4, 20)
	want := date(2026, 4, 13)
	got  := subtractWorkdays(mon, 5)
	if !got.Equal(want) {
		t.Errorf("Mon - 5 workdays = %s, want %s",
			got.Format("02.01"), want.Format("02.01"))
	}
}
...
func assertWeekday(t *testing.T, d time.Time, want time.Weekday) {
	t.Helper()
	if d.Weekday() != want {
		t.Fatalf("test setup: %v is not %s", d.Format("02.01.06"), want)
	}
}
...
func assertDeadline(t *testing.T, event, wantLastDay time.Time) {
	t.Helper()
	want := time.Date(
		wantLastDay.Year(), wantLastDay.Month(), wantLastDay.Day(),
		23, 59, 59, 0, time.UTC,
	)
