...
func (h *Handler) register(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		writeError(w, 409, "Пользователь уже существует или некорректные данные")
		return
	}
	...
	if in.HqID != nil && in.HqPositionID != nil {
		...
		if err == nil {
			...
			if in.MiddleName != "" {
				applicantName += " " + in.MiddleName
			}
			body := "👤 " + applicantName
			if in.Phone != "" {
				body += "\n📞 " + in.Phone
			}
			if in.MemberCardNumber != "" {
...
func (h *Handler) refresh(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeError(w, 401, "Токен истёк или недействителен")
		return
	}
	writeJSON(w, 200, map[string]any{"access_token": acc, "refresh_token": ref})
}
...
func (h *Handler) updateProfile(w http.ResponseWriter, r *http.Request) {
	...
	if in.LastName == "" || in.FirstName == "" {
		writeError(w, 400, "last_name and first_name are required")
		return
	}
	loc := in.MemberCardLocation
	if loc != "with_user" && loc != "in_hq" {
...
func (h *Handler) myRegistrations(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeError(w, 500, "registrations query failed")
		return
	}
	if regs == nil {
		regs = []repo.MyRegistration{}
	}
...
func (h *Handler) myHQUnits(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeError(w, 403, "not a hq staff member")
		return
	}
	if units == nil {
		units = []models.Unit{}
...
func (h *Handler) unitMembers(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeError(w, 400, "invalid unit id")
		return
	}
	members, err := h.svc.ListUnitMembers(r.Context(), unitID)
	if err != nil {
		writeError(w, 500, "members query failed")
		return
	}
	if members == nil {
		members = []models.User{}
...
func (h *Handler) listHQPositions(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeError(w, 500, "hq_positions query failed")
		return
	}
	writeJSON(w, 200, p)
}
...
func (h *Handler) createEvent(w http.ResponseWriter, r *http.Request) {
	...
	if in.Title == "" || in.EventDate == "" || in.StartTime == "" {
		writeError(w, 400, "title, event_date, start_time are required")
		return
	}
	if in.LevelCode == "" {
		in.LevelCode = "local"
	...
	if in.ParticipationMode == "" {
		in.ParticipationMode = "open"
	}
	...
	if err != nil {
		writeError(w, 500, "create event failed: "+err.Error())
		return
	}
...
func (h *Handler) registerToEvent(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeError(w, 500, "event lookup failed")
		return
	}
	...
	if target == nil {
		writeError(w, 404, "Мероприятие не найдено")
		return
	}
	...
	go h.scheduleParticipantsEmail(context.Background(), eid, target.EventDate)
}
...
func (h *Handler) scanAttendance(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, service.ErrInvalidQR):
			writeError(w, 400, "Неверный формат QR-кода")
		case errors.Is(err, repo.ErrAlreadyAttended):
			writeError(w, 409, "Посещение уже отмечено")
		case repo.IsNotFound(err):
			writeError(w, 404, "Регистрация не найдена")
		default:
...
func (h *Handler) setEventQuota(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&in); err != nil || in.UnitID == 0 {
		writeError(w, 400, "unit_id required")
		return
	}
	if err := h.svc.SetEventUnitQuota(r.Context(), models.EventUnitQuota{
		...
		writeError(w, 500, "quota set failed")
		return
	}
	writeJSON(w, 200, map[string]any{"status": "ok"})
}
...
func (h *Handler) hqStaffRequest(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&in); err != nil || in.HQID == 0 || in.PositionID == 0 {
		writeError(w, 400, "hq_id and hq_position_id are required")
		return
	}
...
func (h *Handler) hqStaffPending(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || hqID == 0 {
		writeError(w, 400, "hq_id required")
		return
	}
...
func (h *Handler) hqStaffCheckPosition(w http.ResponseWriter, r *http.Request) {
	...
	if err1 != nil || err2 != nil || hqID == 0 || posID == 0 {
		writeError(w, 400, "hq_id and position_id required")
		return
	}
...
func (h *Handler) scheduleParticipantsEmail(ctx context.Context, eventID int, eventDateStr string) {
	if h.cfg.EmailTo == "" {
		return
	}
	var year, month, day int
	if _, err := fmt.Sscanf(eventDateStr, "%d-%d-%d", &year, &month, &day); err != nil {
		return
	}
	eventDate := time.Date(year, time.Month(month), day, 0, 0, 0, 0, time.UTC)
	lastDay := swdH(eventDate, 3)
...
func swdH(t time.Time, n int) time.Time {
	r := t
	for s := 0; s < n; {
		r = r.AddDate(0, 0, -1)
...
func (h *Handler) sendParticipantsEmail(ctx context.Context, eventID int) {
	...
	var pp []EventParticipant
	for _, r := range rows {
		cardNum := r.CardNumber
		if cardNum == "" && r.CardLocation == "in_hq" {
			cardNum = "В РШ"
		}
		pp = append(pp, EventParticipant{
			...
			IsHQStaff:    r.IsHQStaff,
			Phone:        r.Phone,
		})
	}

	// Сортировка с правильной обработкой кириллицы
	...
	sort.SliceStable(pp, func(i, j int) bool {
		...
		// 4. По должности
		oa := positionSortOrder(a.PositionName, a.IsHQStaff)
		ob := positionSortOrder(b.PositionName, b.IsHQStaff)
		if oa != ob {
			return oa < ob
		}
		// 5. По фамилии
	...
	title := ""
	if len(pp) > 0 {
		title = pp[0].EventTitle
	}
	SendParticipantList(h.cfg, title, pp)
}
